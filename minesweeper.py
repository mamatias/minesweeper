...
class Board:
    ...
    def make_new_board(self):
        ...
        while bombs_planted < self.num_bombs:
            ...
            board[row][col] = 'B' # plant the bomb
            bombs_planted += 1
    ...
    def dig(self, row, col):
        ...
        if self.startTime == None:
            self.startTime = round(time()) # Set time when first move occurs
            self.status = "playing"

        if (row, col) in self.bet: # Only a mark
            return True

        self.dug.add((row, col)) # keep track that we dug here

        if self.board[row][col] == 'B': # Hit a bomb
            self.status = "loser"
            self.endTime = round(time())
            return False

        elif (row,col) in self.bet or (row,col) in self.doubt: # Hit a mark a do nothing
            return True

        elif len(self.dug) == self.dim_size ** 2 - self.num_bombs: # Already solve
            self.status = "winner"
            self.endTime = round(time())
            return True
    ...
    def mark(self, row, col):
        """
        Mark the cell with special flags. Goes from 'no mark' -> 'bet' -> 'doubt' -> 'no mark'
        """
        if (row,col) in self.dug:
            return True

        if (row, col) in self.bet:
            self.bet.remove((row,col))
            self.doubt.add((row,col))
            return True

        if (row, col) in self.doubt:
            self.doubt.remove((row,col))
            return True

        # (row,col) not in self.bet and not in self.doubt
        self.bet.add((row,col))
        return True

    def get_visible_board(self):
        """ 
        Returns an array of arrays with every cell dug showing information
        and capital E for empty or unclicked
        """
        if self.status in ('loser', 'winner'):
            return self.board
        ...
        for row in range(self.dim_size):
            for col in range(self.dim_size):
                ...
                elif (row,col) in self.bet:
                    visible_board[row][col] = 'X'
                elif (row,col) in self.doubt:
                    visible_board[row][col] = 'Q'
                else:
                    visible_board[row][col] = 'E'
...
def play(dim_size=10, num_bombs=10):
    ...
    while len(board.dug) < board.dim_size ** 2 - num_bombs:
        print(board)
        # 0,0 or 0, 0 or 0,    0
        user_command = input("(D)ig, (M)ark, (S)uper dig?: ")
        if user_command not in ("D", "M", "S","d", "m", "s"):
        ...
        if user_command in ("M","m"):
            board.mark(row, col)
            continue

        if user_command in ("S","s"):
            breaker = False
